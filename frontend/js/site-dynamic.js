  ...
  async function loadContentBlocks() {
    try {
      ...
      if (page === "experience.html") {
        applyText(".page-text h1", map.experience_page_title, true);
        var paragraphs = document.querySelectorAll(".page-text p");
        if (paragraphs[0] && map.experience_intro_1) {
          paragraphs[0].textContent = map.experience_intro_1;
        }
        if (paragraphs[1] && map.experience_intro_2) {
          paragraphs[1].textContent = map.experience_intro_2;
        }
        applyText("#dynamicExperienceTitle", map.experience_dynamic_title, false);
        applyText("#dynamicExperienceIntro", map.experience_dynamic_intro, false);
      }

      if (page === "skills.html") {
        applyText("#dynamicSkillDocsTitle", map.skills_docs_title, false);
        applyText("#dynamicSkillDocsIntro", map.skills_docs_intro, false);
      }

      if (page === "ai.html") {
        applyText(".message-wall-intro", map.ai_message_intro || map.message_wall_intro, false);
      }
    } catch (error) {
    ...
    try {
      ...
      container.innerHTML = data.map(function (item) {
        ...
        return ''
          ...
          + '    <div class="dynamic-experience-tags">' + tags.map(buildChip).join("") + "</div>"
          + "  </div>"
